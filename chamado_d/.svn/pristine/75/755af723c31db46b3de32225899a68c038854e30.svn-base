...
abstract class BasePrioridade extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the no_prioridade field.
	 * @var        string
	 */
	protected $no_prioridade;
	...
	/**
	 * Get the [no_prioridade] column value.
	 * 
	 * @return     string
	 */
	public function getNoPrioridade()
	{
		return $this->no_prioridade;
	}
	...
	public function setCoPrioridade($v)
	{
		...
		if ($this->co_prioridade !== $v) {
			...
			$this->modifiedColumns[] = PrioridadePeer::CO_PRIORIDADE;
		}
	...
	/**
	 * Set the value of [no_prioridade] column.
	 * 
	 * @param      string $v new value
	 * @return     Prioridade The current object (for fluent API support)
	 */
	public function setNoPrioridade($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->no_prioridade !== $v) {
			$this->no_prioridade = $v;
			$this->modifiedColumns[] = PrioridadePeer::NO_PRIORIDADE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->co_prioridade = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->no_prioridade = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = PrioridadePeer::NUM_HYDRATE_COLUMNS.
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = PrioridadeQuery::create()
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(PrioridadePeer::CO_PRIORIDADE) ) {
						throw new PropelException('Cannot insert a value for auto-increment primary key ('.PrioridadePeer::CO_PRIORIDADE.')');
					}

					$pk = BasePeer::doInsert($criteria, $con);
					$affectedRows = 1;
					$this->setCoPrioridade($pk);  //[IMV] update autoincrement primary key
					$this->setNew(false);
				} else {
	...
	{
		if (isset($alreadyDumpedObjects['Prioridade'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Prioridade'][$this->getPrimaryKey()] = true;
		$keys = PrioridadePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getCoPrioridade(),
			$keys[1] => $this->getNoPrioridade(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setCoPrioridade($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setNoPrioridade($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PrioridadePeer::CO_PRIORIDADE)) $criteria->add(PrioridadePeer::CO_PRIORIDADE, $this->co_prioridade);
		if ($this->isColumnModified(PrioridadePeer::NO_PRIORIDADE)) $criteria->add(PrioridadePeer::NO_PRIORIDADE, $this->no_prioridade);
	...
	{
		$criteria = new Criteria(PrioridadePeer::DATABASE_NAME);
		$criteria->add(PrioridadePeer::CO_PRIORIDADE, $this->co_prioridade);
	...
	/**
	 * Generic method to set the primary key (co_prioridade column).
	 *
	...
	{
		return null === $this->getCoPrioridade();
	}
	...
	{
		$copyObj->setNoPrioridade($this->getNoPrioridade());
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setCoPrioridade(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 * If this Prioridade is new, it will return
	 ...
	 * Otherwise if this Prioridade is new, it will return
	 * an empty collection; or if this Prioridade has previously
	 * been saved, it will retrieve related Tarefas from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Prioridade.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(PrioridadePeer::DEFAULT_STRING_FORMAT);
	}
