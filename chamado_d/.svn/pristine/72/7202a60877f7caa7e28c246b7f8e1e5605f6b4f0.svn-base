...
/**
 * Base class that represents a row from the 'usuario_orgao' table.
 *
 * 
 *
 * @package    propel.generator.tarefa.om
 */
abstract class BaseUsuarioOrgao extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'UsuarioOrgaoPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        UsuarioOrgaoPeer
	 */
	 ...
	 * @return     UsuarioOrgao The current object (for fluent API support)
	 */
	public function setCoUsuario($v)
	{
		...
		if ($this->co_usuario !== $v) {
			$this->co_usuario = $v;
			$this->modifiedColumns[] = UsuarioOrgaoPeer::CO_USUARIO;
		}
	...
	public function setCoOrgao($v)
	{
		...
		if ($this->co_orgao !== $v) {
			$this->co_orgao = $v;
			$this->modifiedColumns[] = UsuarioOrgaoPeer::CO_ORGAO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = UsuarioOrgaoPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating UsuarioOrgao object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($con === null) {
			$con = Propel::getConnection(UsuarioOrgaoPeer::DATABASE_NAME, Propel::CONNECTION_READ);
		}

		// We don't need to alter the object instance pool; we're just modifying this instance
		// already in the pool.

		$stmt = UsuarioOrgaoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		if ($con === null) {
			$con = Propel::getConnection(UsuarioOrgaoPeer::DATABASE_NAME, Propel::CONNECTION_WRITE);
		}

		$con->beginTransaction();
		try {
			$deleteQuery = UsuarioOrgaoQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				UsuarioOrgaoPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(UsuarioOrgaoPeer::CO_USUARIO) ) {
						throw new PropelException('Cannot insert a value for auto-increment primary key ('.UsuarioOrgaoPeer::CO_USUARIO.')');
					}
					...
					$affectedRows += UsuarioOrgaoPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = UsuarioOrgaoPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = UsuarioOrgaoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['UsuarioOrgao'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['UsuarioOrgao'][serialize($this->getPrimaryKey())] = true;
		$keys = UsuarioOrgaoPeer::getFieldNames($keyType);
		$result = array(
	...
	{
		$criteria = new Criteria(UsuarioOrgaoPeer::DATABASE_NAME);

		if ($this->isColumnModified(UsuarioOrgaoPeer::CO_USUARIO)) $criteria->add(UsuarioOrgaoPeer::CO_USUARIO, $this->co_usuario);
		if ($this->isColumnModified(UsuarioOrgaoPeer::CO_ORGAO)) $criteria->add(UsuarioOrgaoPeer::CO_ORGAO, $this->co_orgao);
	...
	{
		$criteria = new Criteria(UsuarioOrgaoPeer::DATABASE_NAME);
		$criteria->add(UsuarioOrgaoPeer::CO_USUARIO, $this->co_usuario);
		$criteria->add(UsuarioOrgaoPeer::CO_ORGAO, $this->co_orgao);
	...
	{
		return (null === $this->getCoUsuario()) && (null === $this->getCoOrgao());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of UsuarioOrgao (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     UsuarioOrgao Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     UsuarioOrgaoPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new UsuarioOrgaoPeer();
		}
		return self::$peer;
	}
	...
	public function getOrgao(PropelPDO $con = null)
	{
		if ($this->aOrgao === null && ($this->co_orgao !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aOrgao->addUsuarioOrgaos($this);
			 */
		}
		return $this->aOrgao;
	}
	...
	public function getUsuario(PropelPDO $con = null)
	{
		if ($this->aUsuario === null && ($this->co_usuario !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aUsuario->addUsuarioOrgaos($this);
			 */
		}
	...
	{
		return (string) $this->exportTo(UsuarioOrgaoPeer::DEFAULT_STRING_FORMAT);
	}
