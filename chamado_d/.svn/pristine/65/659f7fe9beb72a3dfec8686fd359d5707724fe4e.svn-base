...
abstract class BaseNivel extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the no_nivel field.
	 * @var        string
	 */
	protected $no_nivel;
	...
	/**
	 * Get the [no_nivel] column value.
	 * 
	 * @return     string
	 */
	public function getNoNivel()
	{
		return $this->no_nivel;
	}
	...
	public function setCoNivel($v)
	{
		...
		if ($this->co_nivel !== $v) {
			$this->co_nivel = $v;
			$this->modifiedColumns[] = NivelPeer::CO_NIVEL;
		}
	...
	/**
	 * Set the value of [no_nivel] column.
	 * 
	 * @param      string $v new value
	 * @return     Nivel The current object (for fluent API support)
	 */
	public function setNoNivel($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}
		...
		if ($this->no_nivel !== $v) {
			$this->no_nivel = $v;
			$this->modifiedColumns[] = NivelPeer::NO_NIVEL;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->co_nivel = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->no_nivel = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = NivelPeer::NUM_HYDRATE_COLUMNS.
	...
	{
		if (isset($alreadyDumpedObjects['Nivel'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Nivel'][$this->getPrimaryKey()] = true;
		$keys = NivelPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getCoNivel(),
			$keys[1] => $this->getNoNivel(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setCoNivel($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setNoNivel($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NivelPeer::CO_NIVEL)) $criteria->add(NivelPeer::CO_NIVEL, $this->co_nivel);
		if ($this->isColumnModified(NivelPeer::NO_NIVEL)) $criteria->add(NivelPeer::NO_NIVEL, $this->no_nivel);
	...
	{
		$criteria = new Criteria(NivelPeer::DATABASE_NAME);
		$criteria->add(NivelPeer::CO_NIVEL, $this->co_nivel);
	...
	/**
	 * Generic method to set the primary key (co_nivel column).
	 *
	...
	{
		$copyObj->setNoNivel($this->getNoNivel());
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setCoNivel(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 * If this Nivel is new, it will return
	 ...
	 * Otherwise if this Nivel is new, it will return
	 * an empty collection; or if this Nivel has previously
	 * been saved, it will retrieve related Tarefas from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Nivel.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(NivelPeer::DEFAULT_STRING_FORMAT);
	}
